...
async def start_command(message: types.Message, state: FSMContext):
    ...
    if message_del := state_data.get('description_message'):
        await delete_message_from_chat(message.chat.id, message_del)  # delete old description message
    if message_del := state_data.get('consulting_message'):
        await delete_message_from_chat(message.chat.id, message_del)
